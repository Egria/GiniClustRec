...
def giniClust_preprocess(main_args: argparse.Namespace, expr_params: GiniClust_Parameters) -> pandas.DataFrame:
    ...
    if (main_args.type == 'RNA-seq'):
        exprM_raw_counts = pandas.read_csv(main_args.file, sep=',', header=0, index_col=0).dropna(axis=1, how='all')
        ...
        exprM_log2 = pandas.read_csv(main_args.file, sep=',', header=0, index_col=0).dropna(axis=1, how='all')
